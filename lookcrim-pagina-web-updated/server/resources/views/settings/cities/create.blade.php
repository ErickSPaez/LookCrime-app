...
<div class="main-website-interior user-management-panel">
    ...
    <div style="display:flex;justify-content:flex-end;gap:8px;align-items:center;flex-wrap:wrap;margin:0 0 18px 0;">
        <a class="btn btn-lookcrim btn-sm" href="{{ route('settings.city.index') }}">{{ __('pages.back') }}</a>
    </div>
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('settings.city.store') }}">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input class="form-input" id="radius-km" type="number" name="radius_km" step="0.1" min="0.1" value="{{ old('radius_km', 25) }}" required>
                ...
                <div class="form-group" style="flex:1">
                    ...
                    <input type="hidden" name="center_lat" id="center-lat" value="{{ old('center_lat') }}">
                    <input type="hidden" name="center_lng" id="center-lng" value="{{ old('center_lng') }}">
                </div>
            </div>
            ...
            <div class="form-actions" style="margin-top:14px;">
                ...
                <a class="btn-outline-secondary lc-btn-edit" href="{{ route('settings.city.index') }}">{{ __('pages.cancel') }}</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function(){
    const defaultCenter = [41.1579, -8.6291]; // Porto fallback
    ...
    // Initialize center from current map view if empty.
